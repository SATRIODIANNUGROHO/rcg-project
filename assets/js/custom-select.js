...
const FA_ICONS = {
  ...
  arrowDown: '<svg class="dropdown-chevron" width="11" height="11" viewBox="0 0 384 512" fill="currentColor"><path d="M169.4 470.6c12.5 12.5 32.8 12.5 45.3 0l160-160c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L224 370.8 224 64c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 306.7L54.6 265.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l160 160z"/></svg>'
};
...
const CustomSelectManager = {
  ...
  enhance(select) {
    ...
    // Toggle on trigger click (Always opens downwards)
  ...
  populateMenu(select, menu, trigger, textSpan) {
    ...
    Array.from(select.options).forEach(option => {
      ...
      optItem.addEventListener('click', (e) => {
        ...
        if (wrapper) wrapper.classList.remove('open', 'open-upward');
  ...
  bindGlobalEvents() {
    ...
    window.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        document.querySelectorAll('.custom-select-container.open, .user-profile-dropdown.open').forEach(w => w.classList.remove('open'));
      }
    });
  }
};
